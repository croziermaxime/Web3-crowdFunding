    ...
    return (
        <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
            <div className="lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[100px]">
                ...
                <div
                    className="w-[72px] h-full rounded-[20px] violet-gradient flex justify-center items-center cursor-pointer">
            ...
            <div className="sm:flex hidden flex-row justify-end gap-4">
                <CustomButton
                    ...
                    styles={address ? 'violet-gradient' : 'bg-[#8c6dfd]'}
                    handleClick={() => {
                    ...
                    <ul className="mb-4">
                        {navlinks.map((Link) => (
                            <li key={Link.name} className={`flex p-4 ${isActive === Link.name && 'bg-[3a3a43]'}`}
                                ...
                                />                                <p
                                className={`ml-[20px] font-epilogue font-semibold text-[14px] ${isActive === Link.name ? 'text-[#1dc071]' : 'text-[#808191]'}`}>{Link.name}</p>
                            </li>
                        ))}
                    </ul>
